...
// Invoice saving
export const createInvoice = async (invoiceData: IInvoice): Promise<IInvoice> => {
  const query = `
...
export const updatePaymentSuccess = async (sessionId: string): Promise<boolean> => {
  ...
  try {
    ...
    if (invoice.milestone_id) {
      await client.query(`UPDATE installment_milestones SET status = 'PAID', paid_at = CURRENT_TIMESTAMP, stripe_charge_id = $1 WHERE id = $2`, [sessionId, invoice.milestone_id]);
    }
