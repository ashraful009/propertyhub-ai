...
const generateInstallmentInvoicePDF = ({ booking, installment, property, company, customer }) => {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('rgba(255,255,255,0.6)').fontSize(9)
       .text('House No. 2, Road No. 11, Block F, Banani, Dhaka-1213', 50, 76);

    doc.fillColor('#ffffff').fontSize(20).font('Helvetica-Bold')
       .text('INSTALLMENT RECEIPT', 350, 32, { align: 'right', width: 195 });
    doc.fillColor('rgba(255,255,255,0.6)').fontSize(9).font('Helvetica')
       .text(`Receipt #: ${shortId(installment._id)}`, 350, 60, { align: 'right', width: 195 })
       .text(
         `Date: ${fmtDate(installment.paidAt || new Date())}`,
    ...
    const planRows = [
      ...
      ['Total Installments',  installment.totalInstallments],
      ['Extra Charge Tier',   `${installment.extraChargePercentage}% per installment`],
    ];
    planRows.forEach(([label, value], i) => {
      const y = secY + 24 + i * 18;
    ...
    const tableRows = [
      ['Principal (Base Amount)',                                  fmt(installment.baseAmount)],
      [`Installment Service Fee (${installment.extraChargePercentage}%)`, fmt(installment.extraCharge)],
    ];
    if (wasLate) {
      tableRows.push(['Late Payment Fine (overdue after 15th)', fmt(installment.lateFee)]);
    }
    ...
    tableRows.forEach(([desc, amt], i) => {
      ...
      doc.fillColor(TEXT_DARK).fontSize(9).font('Helvetica').text(desc, 62, ry + 7);
      doc.fillColor(wasLate && i === 2 ? WARN : TEXT_DARK).fontSize(9).font('Helvetica-Bold')
    ...
    doc.fillColor(TEXT_DARK).fontSize(9).font('Helvetica-Bold')
       .text(fmtDate(installment.dueDate), 130, dY);
    ...
    const [badgeColor, badgeLabel] = wasLate
      ? [WARN, 'PAID (LATE)']
      : [SUCCESS, 'PAID ON TIME'];
    doc.roundedRect(50, badgeY, 130, 24, 4).fill(badgeColor);
    doc.fillColor('#ffffff').fontSize(9).font('Helvetica-Bold')
...
module.exports = generateInstallmentInvoicePDF;
